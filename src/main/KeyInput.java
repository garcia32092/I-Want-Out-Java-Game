		...
		for (int i = 0; i < handler.object.size(); i++) {
			...
			if (tempObject.getId() == ID.Player ) {
				//key events for Player 1
					...
					if (keyUp && keyRight) tempObject.setDirection("upright");
					if (keyUp && keyLeft) tempObject.setDirection("upleft");
					if (keyDown && keyRight) tempObject.setDirection("downright");
					if (keyDown && keyLeft) tempObject.setDirection("downleft");
			}
		}
